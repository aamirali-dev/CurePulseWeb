...
# Determine the optimal value of `eps` for DBSCAN dynamically
def calculate_optimal_eps(data, min_samples=2):
    sorted_distances = np.sort(np.linalg.norm(data - data[:, np.newaxis], axis=2).flatten())
    max_distance = sorted_distances[-1]
    min_distance = sorted_distances[min_samples - 1]

    # Use the midpoint between min and max distances as a starting point for `eps`
    eps = (max_distance + min_distance) / 2

    return eps

# Calculate the optimal `eps` value
# optimal_eps = calculate_optimal_eps(sentiment_probs)
optimal_eps = 0.01

# Perform DBSCAN clustering with the optimal `eps` value
dbscan = DBSCAN(eps=optimal_eps, min_samples=2)
cluster_assignments = dbscan.fit_predict(sentiment_probs)
...
ax.set_title('Customer Call Sentiment Clustering (DBSCAN)')
plt.legend()
plt.show()

print("Optimal Value of `eps`:", optimal_eps)
